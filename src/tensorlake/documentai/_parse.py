...
class _ParseMixin(_BaseClient):
    ...
    def wait_for_completion(self, parse_id: str):
        ...
        """
        print("Waiting for completion of parse job.", file=sys.stderr)
        retry_count = 0

        print(f"Parse ID: {parse_id}", file=sys.stderr)
        ...
        while retry_count < 5:
            try:
                ...
                retry_count += 1
                print(
                    f"Connection issue (attempt {retry_count} / 5): {e}",
                    file=sys.stderr,
                )
                if retry_count < 5:
