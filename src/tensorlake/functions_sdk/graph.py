...
def _extract_function_parameters(node: TensorlakeCompute) -> Tuple[List[ParameterMetadata], Optional[str]]:
    ...
    if origin:
        if origin is list:
            if args:
                return {
                    "type": "array",
                    "items": _format_type_annotation(args[0])
                }
            else:
...
class Graph:
    ...
    def definition(self) -> ComputeGraphMetadata:
        ...
        start_node_params, start_node_return_type = _extract_function_parameters(start_node)
