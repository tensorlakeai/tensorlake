...
def future_durable_id(
    ...
    future_durable_ids: dict[str, str],
) -> str:
    ...
    future_durable_ids is a mapping from Future IDs to their durable IDs.
    ...
    if isinstance(future, FunctionCallFuture):
        ...
        for arg in future._args:
            _add_future_durable_id(
                value=arg,
                future_durable_ids=future_durable_ids,
        ...
        # Future specific metadata, part of durable ID.
        durable_id_attrs.append(f"MAP_OPERATION:{future._function_name}")
        items: Future | list[_TensorlakeFutureWrapper[Future] | Any] = _unwrap_future(
            future._items
        )
...
def _add_future_durable_id(
    ...
    if isinstance(value, Future):
        value_durable_id: str | None = future_durable_ids.get(value._id, None)
        if value_durable_id is None:
            raise InternalError(
                f"Durable ID for Future with id {value._id} not found in future_durable_ids."
            )
        durable_id_attrs.append(value_durable_id)


def _sha256_hash_strings(strings: list[str]) -> str:
    """Returns sha256 hash of the concatenation of strings in the given list.

    Each string is length-prefixed to unambiguously separate entries and prevent
    collisions (e.g. ["ab", "c"] vs ["a", "bc"] produce different hashes).
    ...
    for s in strings:
        ...
        sha256.update(len(encoded).to_bytes(8, "big"))
