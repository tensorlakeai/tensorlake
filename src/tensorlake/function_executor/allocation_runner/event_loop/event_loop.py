...
class AllocationEventLoop:
    ...
    def __init__(
        ...
        self._completed_future_orders: dict[str, int] = {}
    ...
    def _wait_futures_runtime_hook(
        ...
        if return_when == RETURN_WHEN.FIRST_COMPLETED and len(already_done) > 0:
            return already_done, already_not_done
        if return_when == RETURN_WHEN.FIRST_FAILURE and any(
            future.exception is not None for future in already_done
        ):
            return already_done, already_not_done

        # Keep local references to FutureInfo because a watcher result removes
        # the Future from the active tracking dictionaries as soon as it arrives.
        futures_that_need_watchers: list[Future] = list(already_not_done)
        tracked_infos = self._create_watchers(futures_that_need_watchers, deadline)
        tracked_by_future_id = {
            future._id: future_info for future, future_info in tracked_infos
        }
        pending: list[tuple[Future, _FutureInfo]] = [
            (future, future_info)
            for future, future_info in tracked_infos
            if not future_info.function_call_finished
        ]
        done_infos: list[tuple[Future, int]] = [
            (future, future_info.completion_order)
            for future, future_info in tracked_infos
            if future_info.function_call_finished
        ]
        for future in futures_that_need_watchers:
            if future._id in tracked_by_future_id:
                continue
            completion_order = self._completed_future_orders.get(future._id)
            if completion_order is None:
                raise InternalError(
                    f"Future {future._id} stopped being tracked before its watcher was created"
                )
            done_infos.append((future, completion_order))
        ...
        with self.function_call_completed:
            while True:
                ...
                done_infos.extend(
                    (future, future_info.completion_order)
                    for future, future_info in newly_done
                )
                pending = [
                ...
                if return_when == RETURN_WHEN.FIRST_COMPLETED:
                    ...
                    if any(f.exception is not None for f, _ in done_infos):
                        break
                    if len(pending) == 0:
                        break
        ...
        if return_when == RETURN_WHEN.ALL_COMPLETED or (
            return_when == RETURN_WHEN.FIRST_FAILURE
            and not any(f.exception is not None for f, _ in done_infos)
        ):
            completed_during_wait = {f for f, _ in done_infos}
        else:
            if return_when == RETURN_WHEN.FIRST_COMPLETED:
                completion_cutoff = min(order for _, order in done_infos)
            else:
                completion_cutoff = min(
                    order
                    for future, order in done_infos
                    if future.exception is not None
                )
            completed_during_wait = {
                future for future, order in done_infos if order <= completion_cutoff
            }

        # Preserve the deterministic snapshot taken at wait entry, then include
        # only completions through the event that satisfied return_when. Results
        # delivered after that event must not leak into this wait merely because
        # the input thread ran again before the user thread resumed.
        done_set = set(already_done) | completed_during_wait
        done: list[Future] = [future for future in futures if future in done_set]
        not_done: list[Future] = [
            future for future in futures if future not in done_set
        ]
    ...
    def _create_watchers(
        self, futures: list[Future], deadline: float | None
    ) -> list[tuple[Future, _FutureInfo]]:
        ...
        pending_future_infos: list[_FutureInfo] = []
        tracked_infos: list[tuple[Future, _FutureInfo]] = []
        for future in futures:
            durable_id = self._future_durable_id.get(future._id)
            future_info = (
                None
                if durable_id is None
                else self._future_infos_by_durable_id.get(durable_id)
            )
            if future_info is None:
                if future.done():
                    continue
                raise InternalError(f"Future {future._id} is not tracked")
            tracked_infos.append((future, future_info))
            if not future_info.watcher_creation_started:
                future_info.watcher_creation_started = True
                output_events.append(
                    OutputEventCreateFunctionCallWatcher(
                        function_call_durable_id=durable_id,
                        deadline=deadline,
                    )
                )
                pending_future_infos.append(future_info)

        if len(output_events) > 0:
            self._output_event_queue.put(OutputEventBatch(events=output_events))

        for future_info in pending_future_infos:
            future_info.watcher_created.wait()
            self._check_emergency_shutdown()
        return tracked_infos
    ...
    def _process_input_event_function_call_watcher_created(
        ...
        if event.exception is not None:
            ...
            # A rejected watcher will never produce a watcher-result event. Treat
            # the creation acknowledgement itself as the terminal completion so
            # synchronous waits and async awaits cannot remain blocked forever.
            durable_id = self._future_durable_id.pop(future._id, None)
            if durable_id is not None:
                self._future_infos_by_durable_id.pop(durable_id, None)
            self._complete_function_call(future_info)
        future_info.watcher_created.set()
        # Safe to process next input event immediately: each Future's state is written
        # at most once per event type, and subsequent events operate on different Futures.
    ...
    def _process_input_event_function_call_watcher_result(
        ...
        # Record completion order before removing the active lookup entries.
        # Future.wait can observe the Future as done concurrently and needs to
        # recover this order even after the active entries disappear.
        self._assign_function_call_completion_order(future_info)
    ...
    def _assign_function_call_completion_order(self, future_info: _FutureInfo) -> None:
        if future_info.completion_order >= 0:
            return
        future_info.completion_order = self.function_call_completion_counter
        self._completed_future_orders[future_info.future._id] = (
            future_info.completion_order
        )
