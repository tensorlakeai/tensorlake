...
class MessageValidator:
    ...
    def repeated_field_with_items(self, field_name: str) -> "MessageValidator":
        """Validates that the field is repeated and has at least one item.

        Raises: ValueError: If this is not true.
        """
        if len(getattr(self._message, field_name)) == 0:
            raise ValueError(
                f"Field '{field_name}' must have at least one item in {type(self._message).__name__}"
            )
        return self
