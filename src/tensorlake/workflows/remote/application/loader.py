...
def load_application(code_dir_or_filepath: str, ignored_paths: Set[str]) -> None:
    """Loads all Python files from the application code directory or file."""
    code_dir_path: str = (
    ...
    for py_file_absolute_path in walk_application_code(
        ...
        py_file_path_inside_code_dir: str = os.path.relpath(
            py_file_absolute_path, code_dir_or_filepath
        )
...
def walk_application_code(code_dir_or_file_path: str, ignored_absolute_paths: Set[str]):
    """Yields all absolute Python file paths from the application code directory or just the supplied file path."""
