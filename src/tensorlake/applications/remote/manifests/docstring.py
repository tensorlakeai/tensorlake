...
def detect_docstring_style(docstring: str) -> DocstringStyle:
    ...
    # 1. reStructuredText (Sphinx) Heuristics
    ...
    numpy_header_pattern = re.compile(
    ...
    google_header_pattern = re.compile(
    ...
    rst_score = len(rst_pattern.findall(docstring))
    # Weighted higher because headers are rare/distinct indicators
    numpy_score = len(numpy_header_pattern.findall(docstring)) * 2
    google_score = len(google_header_pattern.findall(docstring)) * 2
