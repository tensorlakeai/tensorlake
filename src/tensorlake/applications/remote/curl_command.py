...
def example_application_curl_command(
    ...
    if len(parameters) == 0:
        ...
        param_manifest: ParameterManifest = app_func_manifest.parameters[0]

        param_definition, _ = _render_parameter(param, param_manifest, file_paths)
        curl_param_definition_lines.append(param_definition)
        curl_command_lines.append(f"--json ${param.name}_value")
    else:
        curl_command_lines.append('-H "Accept: application/json"')
        ...
        for param_ix in range(len(parameters)):
            ...
            param_definition, param_reference = _render_parameter(
...
def _render_parameter(
    ...
    if is_file_type_hint(param.annotation):
        ...
        if file_paths is not None and param.name in file_paths:
            return (
                f"{param_definition_name}='@{file_paths[param.name]}'",
                f"-F {param.name}=${param_definition_name}",
            )
        else:
            # Always render File params even if they have default values because we can't render
            # File default values in curl command.
            return (
                f"{param_definition_name}='@FILE_PATH'",
                f"-F {param.name}=${param_definition_name}",
            )
    else:
        if param_manifest.required:
            return (
                f"{param_definition_name}='"
                + _curl_escape_json_string(fake_json(param.annotation))
                + "'",
            ...
            return (
                f"{param_definition_name}='"
                + _curl_escape_json_string(json.dumps(param_manifest.data_type.default))
                + "'",
                f'-F "{param.name}=${param_definition_name};type=application/json"',
            )


def _curl_escape_json_string(s: str) -> str:
    """Escapes a JSON string for inclusion in a cURL command."""
