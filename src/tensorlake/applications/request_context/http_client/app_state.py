...
from ...interface.request_context import ApplicationState
from ..http_server.handlers.app_state.commit_write import (
    COMMIT_WRITE_PATH,
    COMMIT_WRITE_VERB,
    CommitWriteRequest,
)
from ..http_server.handlers.app_state.prepare_read import (
    PREPARE_READ_PATH,
    PREPARE_READ_VERB,
    PrepareReadRequest,
    PrepareReadResponse,
)
from ..http_server.handlers.app_state.prepare_write import (
    PREPARE_WRITE_PATH,
    PREPARE_WRITE_VERB,
    PrepareWriteRequest,
...
class ApplicationStateHTTPClient(ApplicationState):
    """HTTP client for accessing application state in subprocesses and child threads.
    ...
    def set(self, key: str, value: Any) -> None:
        ...
        try:
            ...
            raise InternalError(f"Failed to set app state for key '{key}'.")
    ...
    def get(self, key: str, default: Any | None = None) -> Any | None:
        ...
        try:
            ...
            raise InternalError(f"Failed to get app state for key '{key}'.")
    ...
    def __getstate__(self):
        raise SDKUsageError("Pickling of ApplicationStateHTTPClient is not supported.")

    def __setstate__(self, state):
        raise SDKUsageError(
            "Unpickling of ApplicationStateHTTPClient is not supported."
        )
