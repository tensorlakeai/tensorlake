...
export class FunctionExecutorService {
  ...
  private createAllocation(call: UnaryCall, callback: Callback): void {
    try {
      ...
      if (!id || !allocation?.requestId || !allocation?.functionCallId || allocation.inputs == null) {
        throw rpcError(grpc.status.INVALID_ARGUMENT, "Allocation is missing required fields");
      }
      ...
      this.log("error", "create allocation RPC failed", this.allocationLogFields(call.request.allocation), error);
