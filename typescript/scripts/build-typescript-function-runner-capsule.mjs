...
mkdirSync(path.join(capsulePackage, "dist", "function-agent"), {
  recursive: true,
});
...
const capsulePackageJson = {
  ...
  description:
    "Tensorlake Node.js function runner with a platform-specific Rust agent core",
  type: "module",
  bin: {
    "tensorlake-typescript-function-runner":
      "./bin/tensorlake-typescript-function-runner.js",
  },
  engines: { node: ">=24.0.0" },
  dependencies,
...
const lockedProductionPackages = Object.fromEntries(
  Object.entries(packageLock.packages)
    .filter(
      ([packagePath, metadata]) => packagePath !== "" && metadata.dev !== true,
    )
...
for (const relativePath of collectFiles(capsulePackage)) {
  ...
  files[`package/${relativePath}`] = {
    ...
    mode:
      relativePath === "bin/tensorlake-typescript-function-runner.js"
        ? 0o755
        : 0o644,
  };
}
...
for (const [file, metadata] of Object.entries(files)) {
  runtimeHash.update(
    `${file}\0${metadata.sha256}\0${metadata.size}\0${metadata.mode}\n`,
  );
}
...
function collectFiles(directory, prefix = "") {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true }).sort(
    ...
    if (entry.isDirectory())
      ...
      throw new Error(
        `Function runner capsule cannot contain a symlink: ${relativePath}`,
      );
  }
  return files.sort();
}
