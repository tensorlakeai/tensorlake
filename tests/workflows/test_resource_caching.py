...
class TestFileDescriptorCaching(unittest.TestCase):
    ...
    def test_second_write_goes_to_cached_file_descriptor_if_same_func(self):
        ...
        request = tensorlake.call_remote_api(fd_caching_function, "write_fd")
        output: str = request.output()
        self.assertEqual(output, "success")
        ...
        request = tensorlake.call_remote_api(fd_caching_function, "read_fd")
        output: str = request.output()
...
class TestFunctionClassInstanceCaching(unittest.TestCase):
    ...
    def test_function_class_instance_caching(self):
        ...
        for i in range(5):
            ...
            request: tensorlake.Request = tensorlake.call_remote_api(
                FunctionClass.run, "check"
            )
            output: str = request.output()
            self.assertEqual(output, "success")

            # Every new request will reuse the cached compute object.
            request = tensorlake.call_remote_api(FunctionClass.run, "check")
