...
from tensorlake.workflows.application import define_default_application
from tensorlake.workflows.registry import get_functions
from tensorlake.workflows.remote.application.application import (
    ApplicationManifest,
...
from tensorlake.workflows.remote.application.function_resources import (
...
@tensorlake.function(timeout=10)
def function_with_custom_timeout(x: int) -> str:
    return "success"


@tensorlake.cls(init_timeout=20)
...
class TestFunctionManifestRetries(unittest.TestCase):
    ...
    def test_custom_application_retries(self):
        ...
        app_manifest: ApplicationManifest = create_application_manifest(
            app=tensorlake.define_application(
...
@tensorlake.function(cpu=2.25, memory=2, ephemeral_disk=10, gpu="H100")
def function_with_custom_resources(x: int) -> str:
    return "success"


@tensorlake.function(gpu="A100-40GB:4")
def function_with_many_gpus_per_model(x: int) -> str:
    return "success"


@tensorlake.function(gpu=["A100-40GB:4", "H100:2", "A100-80GB:1"])
...
@tensorlake.function(max_concurrency=5)
