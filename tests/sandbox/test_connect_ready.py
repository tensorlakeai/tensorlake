...
def info(status=SandboxStatus.RUNNING, route="https://sandbox.test"):
...
def test_connect_waits_for_health_and_refreshes_routing():
...
def test_connect_resumes_even_without_a_suspended_proxy_url():
    client, handle = client_and_handle()
...
def test_concurrent_resume_waits_for_the_winner():
    client, handle = client_and_handle()
...
def test_resume_quota_failure_is_not_retried():
    client, handle = client_and_handle()
    ...
    with patch("tensorlake.sandbox.client.SandboxClient", return_value=client):
        with pytest.raises(RemoteAPIError, match="quota exceeded"):
            Sandbox.connect("sandbox-id")
    client.resume.assert_called_once()
    handle.health.assert_not_called()


def test_passive_connect_does_not_read_or_wake():
    client, handle = client_and_handle()
    with patch("tensorlake.sandbox.client.SandboxClient", return_value=client):
...
def test_terminated_connect_never_resumes():
    client, handle = client_and_handle()
...
def test_health_auth_failure_is_not_retried():
    client, handle = client_and_handle()
    handle.health.side_effect = RemoteAPIError(403, "forbidden")
    with patch("tensorlake.sandbox.client.SandboxClient", return_value=client):
        with pytest.raises(RemoteAPIError, match="forbidden"):
            Sandbox.connect("sandbox-id")
    handle.health.assert_called_once()


def test_unready_connect_has_a_deadline():
    client, handle = client_and_handle()
    handle.health.side_effect = RemoteAPIError(503, "not ready")
    with patch("tensorlake.sandbox.client.SandboxClient", return_value=client):
        with pytest.raises(SandboxError, match="within 0.01s"):
            Sandbox.connect("sandbox-id", request_timeout=0.01)


@pytest.mark.parametrize("status", [400, 401, 403, 404, 409, 422, 500])
def test_unrelated_health_errors_are_not_retried(status):
    assert not retryable_health_error(RemoteAPIError(status, "unrelated failure"))


def test_transition_error_is_retryable_only_for_health():
    assert retryable_health_error(
        RemoteAPIError(400, json.dumps({"code": "SANDBOX_NOT_RUNNING"}))
    )


def test_async_connect_resumes_and_probes_without_executing_work():
    import asyncio
...
def test_async_connect_opt_out_is_passive():
    import asyncio
    ...
    with patch(
        ...
        assert asyncio.run(AsyncSandbox.connect("sandbox-id", resume=False)) is handle
...
def test_stale_health_timeout_refreshes_route_and_preserves_command_timeout():
    ...
    assert all(call.args == ("sandbox-id",) for call in client.get.call_args_list[1:])
    handle.run.assert_not_called()


def test_unhealthy_daemon_does_not_complete_connect():
    client, handle = client_and_handle()
    handle.health.side_effect = [
