...
def run_task(
    ...
    if result is None:
        # Check in case if stream finished without task_result.
        raise Exception("Task result was not received from the server.")
    elif not result.HasField("outcome_code"):
        # Check in case if received task_result is empty (not usable for whatever reason).
        raise Exception("Received empty TaskResult.")
    else:
        return result
