...
enum Commands {
    ...
    /// Manage filesystems
    #[command(subcommand, name = "filesystem")]
...
enum FilesystemCommands {
    ...
    Create {
        ...
        /// Print the created filesystem as JSON
        #[arg(long)]
        json: bool,
    },
    ...
    #[command(name = "rm")]
    Rm {
        /// Filesystem id (e.g. `file_system_...`)
...
enum SbxCommands {
    ...
    Create {
        ...
        /// Mount a registered filesystem at boot as
    ...
    /// Attach, detach, or list filesystems on a sandbox
    #[command(subcommand, name = "filesystem")]
    Filesystem(SbxFilesystemCommands),
}

#[derive(Subcommand)]
enum SbxFilesystemCommands {
    /// Attach a registered filesystem to a running sandbox
    Attach {
        /// Sandbox ID or name
        sandbox_id: String,

        /// Filesystem id to attach (e.g. `file_system_...`)
        #[arg(short, long = "id")]
        file_system_id: String,
    ...
    /// Detach the filesystem mounted at a path from a running sandbox
    Detach {
        /// Sandbox ID or name
        sandbox_id: String,
    ...
    /// List filesystems currently mounted on a sandbox
    #[command(name = "ls")]
    Ls {
        /// Sandbox ID or name
        sandbox_id: String,
        ...
        Commands::Filesystem(subcmd) => {
            ...
            match subcmd {
                FilesystemCommands::Create {
                    ...
                    description,
                    json,
                } => commands::filesystem::create(ctx, &name, description.as_deref(), json).await,
                FilesystemCommands::Ls { json } => commands::filesystem::list(ctx, json).await,
                FilesystemCommands::Rm { file_system_id } => {
                    commands::filesystem::remove(ctx, &file_system_id).await
                }
            }
        }
        ...
        Commands::Sbx(subcmd) => match subcmd {
            ...
            other => {
                ...
                match other {
                    ...
                    SbxCommands::Filesystem(fs_cmd) => match fs_cmd {
                        SbxFilesystemCommands::Attach {
                            sandbox_id,
                            file_system_id,
                            path,
                            json,
                        } => {
                            commands::sbx::filesystem::attach(
                                ctx,
                                &sandbox_id,
                                &file_system_id,
                                &path,
                        ...
                        SbxFilesystemCommands::Detach {
                            sandbox_id,
                            path,
                            json,
                        } => commands::sbx::filesystem::detach(ctx, &sandbox_id, &path, json).await,
                        SbxFilesystemCommands::Ls { sandbox_id, json } => {
                            commands::sbx::filesystem::list(ctx, &sandbox_id, json).await
                        }
                    },
                    SbxCommands::Tunnel {
                        sandbox_id,
