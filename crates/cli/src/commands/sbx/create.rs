...
pub struct CreateArgs<'a> {
    ...
    /// Boot-time filesystem mounts, each as `<file_system_id>:<mount_path>`.
    pub filesystems: &'a [String],
}

/// Parse `--filesystem <id>:<path>` flags into the request
/// `file_systems` array. Splits on the first `:` (filesystem ids and
/// absolute mount paths never contain one).
fn parse_filesystem_mounts(raw: &[String]) -> Result<Vec<serde_json::Value>> {
    raw.iter()
        .map(|entry| {
...
pub async fn run(ctx: &CliContext, args: CreateArgs<'_>) -> Result<()> {
    ...
    let filesystem_mounts = parse_filesystem_mounts(filesystems)?;
    if !filesystem_mounts.is_empty() {
        body["file_systems"] = serde_json::Value::Array(filesystem_mounts);
    }
...
mod tests {
    use super::{
        ...
        parse_filesystem_mounts, post_create_proxy_base,
    ...
    #[test]
    fn parse_filesystem_mounts_builds_wire_objects() {
        let raw = vec![
            "file_system_abc:/mnt/skills".to_string(),
            "file_system_def:/data".to_string(),
        ];
        let mounts = parse_filesystem_mounts(&raw).unwrap();
    ...
    #[test]
    fn parse_filesystem_mounts_rejects_missing_colon() {
        assert!(parse_filesystem_mounts(&["file_system_abc".to_string()]).is_err());
    }

    #[test]
    fn parse_filesystem_mounts_rejects_empty_sides() {
        assert!(parse_filesystem_mounts(&[":/mnt".to_string()]).is_err());
        assert!(parse_filesystem_mounts(&["file_system_abc:".to_string()]).is_err());
    }
