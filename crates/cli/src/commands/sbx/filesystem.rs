//! `tl sbx filesystem` — attach/detach/list filesystems on a specific
//! sandbox.
//!
...
//! filesystem registry. Registering and deleting filesystems
//! themselves lives under the top-level `tl filesystem` command.

use comfy_table::Cell;
use reqwest::Response;
use tensorlake::sandboxes::models::{SandboxInfo, FilesystemMount};
...
/// Render a sandbox's currently-mounted filesystems as a table.
fn print_mounts_table(mounts: &[FilesystemMount]) {
    ...
    let mut table = new_table(&["Filesystem ID", "Mount Path"]);
    for mount in mounts {
        table.add_row(vec![
...
pub async fn attach(
    ...
    let info = parse_sandbox_response(resp, "attach filesystem").await?;

    if output_json {
        println!("{}", serde_json::to_string_pretty(&info.filesystems)?);
        return Ok(());
    }

    println!(
        "Attached filesystem '{}' at '{}' on sandbox {}.",
        file_system_id, mount_path, sandbox_id
    );
    print_mounts_table(&info.filesystems);
    Ok(())
}
...
pub async fn detach(
    ...
    let resp = client
        ...
        .await
        .map_err(CliError::Http)?;
    let info = parse_sandbox_response(resp, "detach filesystem").await?;
    ...
    println!(
        "Detached filesystem at '{}' from sandbox {}.",
...
pub async fn list(ctx: &CliContext, sandbox_id: &str, output_json: bool) -> Result<()> {
    ...
    let info = parse_sandbox_response(resp, "list sandbox filesystems").await?;
