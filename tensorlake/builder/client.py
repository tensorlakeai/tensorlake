...
class ImageBuilderClient:
    ...
    def from_env(cls):
        ...
        build_url = os.getenv("TENSORLAKE_BUILD_SERVICE", f"{indexify_url}/images") # Mainly used for debugging/local testing
    ...
    def find_build(self, image_name:str, image_hash:str):
        ...
        res = self.client.get(f"{self.build_service}/v1/builds", headers=self.headers, params=params)
        res.raise_for_status()
        return [Build.model_validate(b) for b in res.json()]

    def get_latest_build(self, image_name:str)->Build:
        res = self.client.get(f"{self.build_service}/v1/builds", headers=self.headers, params={"image_name":image_name})
        res.raise_for_status()
